...
const useProduct = (product) => {
    ...
    const onWishlistHandler = () => {
        ...
        !isInWishlist ? toast(`"${product?.title}" is successfully added.`) : toast.error(`"${product?.title}" is removed.`);
    };

    const onCompareHandler = () => {
        !isInCompareList ? dispatch(addToCompareAction(product)) : dispatch(removeCompareAction(product));
        !isInCompareList ? toast(`"${product?.title}" is successfully added.`) : toast.error(`"${product?.title}" is removed.`);
    };
